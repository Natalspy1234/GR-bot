...
async def handle_reopen(interaction: discord.Interaction, reason: str, message: discord.Message = None):
    try:
        ...
        if logs_channel:
            try:
                log_embed = discord.Embed(title="Report Reopened", color=discord.Color.gold(), timestamp=discord.utils.utcnow())
        ...
        print(f"CRITICAL ERROR in handle_reopen: {e}", exc_info=True)
        try:
            await interaction.followup.send("❌ An error occurred while reopening the report.", ephemeral=True)
        except:
            pass
